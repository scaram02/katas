...
function i(word) {
    ...
    let cons = 'aeiou'
    const vowLength = word.toLowerCase().split('').filter((letter) => cons.includes(letter)).length 
    const consLength = word.length - vowLength
   
    
    const firstLetter = word[0]
    
    if ((vowLength >= consLength) || (firstLetter.toLowerCase() === firstLetter) || (firstLetter.toLowerCase() === "i")) return "Invalid word"
